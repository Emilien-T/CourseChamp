      ...
      <div class="form-container">
        <form @submit.prevent="submitReview">
          <div class="form-group">
            <label class="input-label">Enter your new rating:</label>
            <input type="text" id="rating" v-model="rating" required placeholder="Ex: 5">
          </div>
          <div class="form-group">
            <label class="input-label">Enter your new review:</label>
            <textarea id="text" v-model="text" required placeholder="Ex: I loved this class. It's a must take in U1! "></textarea>
          </div>
          <div class="form-group">
            <label class="input-label">Confirm the semester for which you are rating the course:</label>
  ...
  export default {
    data() {
      return {
        ...
        courseCode: reviewId.courseCode,
        rating: 'reviewId.rating',
        text: reviewId.text,
        semester: reviewId.semester,
        msg: ''
      };
    },
    methods: {
      redirectToStudentHome() {
        ...
        axiosClient.put('/review/'+this.reviewId, reviewData).then(response =>{
          this.msg = `Review modified successfully!`
        }
        ).catch(error =>{
