...
public class Course {
  ...
  // Course Associations
  @ManyToMany
  @JoinTable(name = "Course_CourseOfferings", joinColumns = { @JoinColumn(name = "courseCode") }, inverseJoinColumns = {
      @JoinColumn(name = "id") })
  ...
  public boolean addCourseOfferingAt(CourseOffering aCourseOffering, int index) {
    ...
    if (addCourseOffering(aCourseOffering)) {
      ...
      if (index > numberOfCourseOfferings()) {
        index = numberOfCourseOfferings() - 1;
      }
      courseOfferings.remove(aCourseOffering);
