...
public class AdminController {
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "404", description = "Admin not found.", content = {@Content(mediaType = "String")})
    })
    ...
    @ApiResponses(value = {
        ...
        @ApiResponse(responseCode = "409", description = "Admin account with this email already exists.", content = {@Content(mediaType = "String")})
    })
    @PostMapping("/admin/create")
    public ResponseEntity<AdminResponseDto> createAdmin(@Valid @RequestBody AdminRequestDto AdminRequest)
    {
        Admin Admin = AdminRequest.toModel(); // 1. You pass in a request, validates the constraints, creates an Admin if they pass
        Admin =  adminService.createAdminAccount(Admin); // 2. You use the service class to check if it exists and save it
        AdminResponseDto responseBody = new AdminResponseDto(Admin);
        return new ResponseEntity<AdminResponseDto>(responseBody, HttpStatus.CREATED); //3. You mask the model by returning a Response
    }

    /**
     * Controller Method that allows the verfication of the email and password of an Admin
     * 
     ...
     */
    @GetMapping("/login")
    public ResponseEntity<AdminResponseDto> LoginUser(@RequestParam String email, @RequestParam String password){
        //TODO
        return null;
    }
}
